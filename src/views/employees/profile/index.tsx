...
export function EmployeeProfileView() {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            {/* ── Employee CV / Resume — opened from the top action button ── */}
            {cv && (
                <Dialog open={cvOpen} onOpenChange={setCvOpen}>
                    <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
                        <DialogHeader>
                            <DialogTitle>السيرة الذاتية — {employee.name}</DialogTitle>
                            <DialogDescription>
                                {employee.role} • {employee.subDepartmentName}
                            </DialogDescription>
                        </DialogHeader>
                        <CvContent cv={cv} employee={employee} formatDate={formatDate} />
                    </DialogContent>
                </Dialog>
            )}
